...
const app=getApps().find(a=>a.name==='sfh-photo-fit-quality-v2')||initializeApp(window.SFH_FIREBASE_CONFIG,{name:'sfh-photo-fit-quality-v2'});
...
function injectCss(){
  ...
  s.textContent=`
    ...
    .collectionPortal .photoSlot .sfh-photo{
      ...
      padding:8px!important;box-sizing:border-box!important;background:transparent!important;
      filter:none!important;transform:none!important;
    }
    .sfh-drop,.sfh-portal-photo,.collectionPortal .photoSlot{
...
function fileToDataUrl(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||''));r.onerror=()=>reject(new Error('File could not be read'));r.readAsDataURL(file)})}
...
async function qualityResize(file){
  ...
  const maxSide=2200,scale=Math.min(1,maxSide/Math.max(w,h));
  ...
  let q=.92,out=c.toDataURL('image/jpeg',q);
  while(out.length>MAX_DATA_URL_CHARS&&q>.55){q-=.04;out=c.toDataURL('image/jpeg',q)}
  return out;
}

function updateAdminUi(key,index,data){
  const drop=document.querySelector(`[data-drop="${key}:${index}"]`);const img=drop?.querySelector('img:first-child');if(img)img.src=data;
  const card=drop?.closest('.sfh-slot');const status=card?.querySelector('.sfh-status');if(status){status.textContent='✓ Full photo preserved • high-quality upload active';status.classList.add('active')}
  const chip=card?.querySelector('.sfh-slot-head span');if(chip){chip.textContent='CUSTOM';chip.className='custom'}
}

async function saveFirebase(key,index,dataUrl){
  try{await setDoc(doc(db,FIREBASE_COLLECTION,`${key}-${index+1}`),{portal:key,slot:index+1,dataUrl,updatedAt:serverTimestamp(),updatedBy:ADMIN_EMAIL,photoFit:'contain',maxSide:2200},{merge:true});return true}
  catch(e){console.warn('SFH high-quality Firebase save:',e);return false}
}

function bindUploadQuality(){
  if(window.__sfhPhotoQualityBound)return;window.__sfhPhotoQualityBound=true;
  document.addEventListener('change',e=>{
    const input=e.target?.closest?.('[data-file]');if(!input?.files?.[0])return;
    const [key,indexText]=(input.dataset.file||'').split(':');const index=Number(indexText);if(!key||!Number.isInteger(index)||index<0||index>3)return;
    const file=input.files[0];
    setTimeout(async()=>{
      try{const data=await qualityResize(file);setLocal(key,index,data);updateAdminUi(key,index,data);await saveFirebase(key,index,data)}
...
function refreshStore(){
  ...
  Object.entries(map).forEach(([key,idx])=>{const block=document.querySelectorAll('.collectionBlock')[idx];if(!block)return;[...block.querySelectorAll('.photoSlot')].slice(0,4).forEach((slot,i)=>{const src=all[key]?.[i],img=slot.querySelector('.sfh-photo');if(src&&img)img.src=src})});
  Object.keys(all).forEach(key=>{const card=document.getElementById(`sfh-portal-${key}`);if(!card)return;[...card.querySelectorAll('.sfh-portal-photo .sfh-pp')].slice(0,4).forEach((img,i)=>{const src=all[key]?.[i];if(src)img.src=src})});
}
