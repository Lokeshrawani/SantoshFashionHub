...
(function(){
  ...
  function url(id,slot){
    ...
    return `https://res.cloudinary.com/${encodeURIComponent(String(C.cloudName).trim())}/image/upload/f_auto,q_auto/${C.folder||'sfh-products'}/${safe}/${s}.jpg`;
  }
  function visual(p){
    const direct=(typeof p.image==='string'&&/^https:\/\//i.test(p.image))?p.image:'';
    const u=direct||url(p.id,0);
