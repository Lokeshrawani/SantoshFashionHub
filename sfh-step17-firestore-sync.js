...
*/
(async function(){
  const CFG_READY=()=>{ const c=window.SFH_FIREBASE_CONFIG; return c && Object.values(c).every(v=>typeof v==='string'&&v.trim()&&!/^YOUR_/i.test(v)); };
  if(!CFG_READY()){ window.SFH_FIRESTORE_STATUS='not-configured'; return; }
  try{
    ...
    const {getFirestore,collection,addDoc,doc,updateDoc,onSnapshot,serverTimestamp,query,orderBy,where}=await import('https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js');
    ...
    function localOrder(data,total){
      ...
      return {id,uid:auth.currentUser?.uid||'',customer:data.name,mobile:data.mobile,pin:data.pin,city:data.city,address:data.address,payment:data.payment,note:data.note||'',status:'Pending',total:Number(total||0),createdAt:new Date().toISOString(),items:(window.cart||[]).map(x=>{const p=(window.products||[]).find(y=>y.id===x.id);return p?{id:p.id,name:p.name,size:x.size,qty:Number(x.qty||1),price:Number(p.price||0),total:Number(p.price||0)*Number(x.qty||0)}:null}).filter(Boolean)};
    }
    window.sfhCreateOrder=function(data,cartData){
      ...
      if(auth.currentUser){
        addDoc(collection(db,'orders'),{...order,createdAt:serverTimestamp(),uid:auth.currentUser.uid}).then(ref=>{
          ...
          try{const arr=JSON.parse(localStorage.getItem('sfh-orders')||'[]');const i=arr.findIndex(x=>x.id===order.id);if(i>=0){arr[i].firestoreId=ref.id;localStorage.setItem('sfh-orders',JSON.stringify(arr));}}catch(e){}
        }).catch(err=>{console.warn('SFH Firestore order sync failed:',err);window.SFH_FIRESTORE_STATUS='write-failed';});
      } else window.SFH_FIRESTORE_STATUS='login-required';
      return order;
    };
    window.sfhLoadMyOrders=async function(){
      if(!auth.currentUser)return [];
      return new Promise((resolve,reject)=>{
        const q=query(collection(db,'orders'),where('uid','==',auth.currentUser.uid),orderBy('createdAt','desc'));
        const stop=onSnapshot(q,snap=>{stop();resolve(snap.docs.map(d=>({firestoreId:d.id,...d.data()})))},reject);
      });
    };
    window.sfhWatchOrder=async function(orderId,cb){
      const local=JSON.parse(localStorage.getItem('sfh-orders')||'[]'); const found=local.find(o=>o.id===orderId); const firestoreId=found?.firestoreId;
  ...
  }catch(e){console.warn('SFH Firestore sync unavailable:',e);window.SFH_FIRESTORE_STATUS='error';}
})();
